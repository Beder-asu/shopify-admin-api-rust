...
impl Client {
    ...
    /// Shared loop for handling 429 retries and throttling.
    async fn send_graphql_payload<T: DeserializeOwned>(
        &self,
    ...
    /// Execute a generic GraphQL query or mutation.
    ///
    ...
    pub async fn graphql<T: DeserializeOwned, V: Serialize>(
        ...
        self.send_graphql_payload(&request_payload).await
    }

    /// Execute a strongly-typed GraphQL query generated by `graphql_client`.
    pub async fn send_query<Q: graphql_client::GraphQLQuery>(
        &self,
        variables: Q::Variables,
    ) -> Result<GraphQLResponse<Q::ResponseData>> {
        let payload = Q::build_query(variables);
        self.send_graphql_payload(&payload).await
    }
